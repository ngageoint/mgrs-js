...
export class GridZone {
            ...
            if (drawBounds) {
                ...
                for (let easting = drawBounds
                    ...
                    for (let northing = drawBounds
                        ...
                        const northingPrecision = GridTypeUtils
                            .getPrecision(northing);

                        let southwest = UTM.point(zoneNumber, hemisphere,
                            easting, northing);
                        let northwest = UTM.point(zoneNumber, hemisphere,
                            easting, northing + precision);
                        let southeast = UTM.point(zoneNumber, hemisphere,
                        ...
                        if (precision > 1) {
                            if (southwest.getLongitude() < minLon) {
                                southwest = this.getWestBoundsPoint(easting,
                                    northing, southwest, southeast);
                            } else if (southeast.getLongitude() > maxLon) {
                                southeast = this.getEastBoundsPoint(easting,
                                    northing, southwest, southeast);
                            }
                        }

                        // Vertical line
                        lines.push(GridLine.lineFromPoints(southwest, northwest,
                            eastingPrecision));

                        // Horizontal line
                        lines.push(GridLine.lineFromPoints(southwest, southeast,
                            northingPrecision));
     ...
     */
    private getWestBoundsPoint(easting: number, northing: number,
        west: Point, east: Point): Point {
     ...
     * @return higher precision point
     */
    private getEastBoundsPoint(easting: number, northing: number,
        west: Point, east: Point): Point {
     ...
     * @return higher precision point
     */
    private getBoundsPoint(easting: number, northing: number, west: Point,
        east: Point, eastern: boolean): Point {
        ...
        const boundsPoint = UTM.point(zoneNumber, hemisphere,
    ...
    public getDrawBounds(tileBounds: Bounds, gridType: GridType): Bounds | undefined {
        ...
        if (tileBounds && !tileBounds.isEmpty()) {
            ...
            const upperLeftUTM = UTM.from(tileBounds.getNorthwest(), zoneNumber,
                hemisphere);
            const lowerLeftUTM = UTM.from(tileBounds.getSouthwest(), zoneNumber,
                hemisphere);
            const lowerRightUTM = UTM.from(tileBounds.getSoutheast(), zoneNumber,
                hemisphere);
            const upperRightUTM = UTM.from(tileBounds.getNortheast(), zoneNumber,
                hemisphere);

            const precision = gridType;
            const leftEasting = Math.floor(Math.min(upperLeftUTM.getEasting(),
                lowerLeftUTM.getEasting()) / precision) * precision;
            const lowerNorthing = Math
                .floor(Math.min(lowerLeftUTM.getNorthing(),
                    lowerRightUTM.getNorthing()) / precision)
                * precision;
            const rightEasting = Math
                .ceil(Math.max(lowerRightUTM.getEasting(),
                    upperRightUTM.getEasting()) / precision)
                * precision;
            const upperNorthing = Math
