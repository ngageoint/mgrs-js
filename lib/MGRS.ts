...
export class MGRS {
     ...
     */
    private static readonly columnLetters = ["ABCDEFGH",
        "JKLMNPQR", "STUVWXYZ"];
     ...
     * @return MGRS
     */
    public static create(zone: number, band: string, easting: number,
        northing: number, column?: string, row?: string): MGRS {
     ...
     *            northing
     */
    constructor(zone: number, band: string, column: string, row: string, easting: number,
        northing: number) {
        this.zone = zone;
        this.band = band;
        this.column = column;
    ...
    /**
     * Get the MGRS coordinate with specified grid precision or one meter precision if not provided
     * 
     * @param type
     *            grid type precision
     * @return MGRS coordinate
     */
    public coordinate(type = GridType.METER): string {
    ...
    public getUTMNorthing(): number {
        ...
        const latBandNorthing = UTM.from(Point.degrees(0, latBand))
            .getNorthing();
    ...
    public static isMGRS(mgrs: string): boolean {
        ...
        return MGRS.mgrsPattern.test(mgrs)
            && !MGRS.mgrsInvalidPattern.test(mgrs);
    }
    ...
    public static parse(mgrs: string): MGRS {
        ...
        const band = matcher.group(2).toUpperCase().charAt(0);
        ...
        if (columnRow) {
            ...
            let easting = 0;
            let northing = 0;
            const location = matcher.group(4);
            if (!location.isEmpty()) {
            ...
            if (location.isEmpty()) {
                ...
                const westBounds = point.getLongitude() < gridSouthwest
                    .getLongitude();
                const southBounds = point.getLatitude() < gridSouthwest
                    .getLatitude();

                if (westBounds || southBounds) {

                    if (westBounds && southBounds) {
                        const northeast = MGRS.create(zone, band, GridType.HUNDRED_KILOMETER,
                            GridType.HUNDRED_KILOMETER, column, row)
                            .toPoint();
                        ...
                        const east = MGRS
                            .create(zone, band, GridType.HUNDRED_KILOMETER,
                                northing, column, row)
                            .toPoint();
                        if (gridBounds.containsPoint(east)) {
                            const intersection = MGRS.getWesternBoundsPoint(gridZone,
                                point, east);
                            mgrsValue = MGRS.from(intersection);
                        }
                    } else if (southBounds) {
                        const north = MGRS.create(zone, band,
                            easting, GridType.HUNDRED_KILOMETER, column, row).toPoint();
     ...
     */
    private static getWesternBoundsPoint(gridZone: GridZone, west: Point,
        east: Point): Point {
        ...
        const boundsPoint = UTM.point(zoneNumber, hemisphere, boundsEasting,
            northing);
     ...
     */
    private static getSouthernBoundsPoint(gridZone: GridZone, south: Point,
        north: Point): Point {
        ...
        // Higher precision point just inside of the bounds
        const boundsPoint = UTM.point(zoneNumber, hemisphere, easting,
            boundsNorthing);
     ...
     * @return the two letter column and row 100k designator
     */
    public static getColumnRowId(easting: number, northing: number,
        zoneNumber: number): string {
